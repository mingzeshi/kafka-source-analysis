...
class GroupCoordinator(val brokerId: Int,
  ...
  def startup(enableMetadataExpiration: Boolean = true) {
    info("Starting up.")
    groupManager.startup(enableMetadataExpiration) // 启动了一个后台线程来删除过期的 group metadata
    isActive.set(true) // GroupCoordinator启动后将isActive设置为true
  ...
  private def doCommitOffsets(group: GroupMetadata,
                              ...
                              responseCallback: immutable.Map[TopicPartition, Errors] => Unit) {
    group.inLock { // 加锁
      if (group.is(Dead)) { // 当前消费组的状态是Dead
        responseCallback(offsetMetadata.mapValues(_ => Errors.UNKNOWN_MEMBER_ID)) // 返回Coordinator无当前消费组成员的Response
        ...
        //  此外，对于事务偏移量提交，我们不需要验证组成员关系和生成
        groupManager.storeOffsets(group, memberId, offsetMetadata, responseCallback, producerId, producerEpoch) // 通过groupManager来存储offset等元数据信息
      } else if (group.is(CompletingRebalance)) { // 此消费组正在Rebalance
        responseCallback(offsetMetadata.mapValues(_ => Errors.REBALANCE_IN_PROGRESS)) // 返回当前消费组正在Rebalance请重试的Response
      } else if (!group.has(memberId)) { // 如果此消费组无此memberId消费成员
        responseCallback(offsetMetadata.mapValues(_ => Errors.UNKNOWN_MEMBER_ID)) // 返回Coordinator无当前消费组成员的Response
        /**
         * generationId: 消费组每次Rebalance重新生成的编号ID；
         *
         * 这个判断是，如果此消费组成员提交offset的时候所带有的generationId并不是最新一次Rebalance的generationId，
         * 说明此memberId消费成员可能并没有参与Rebalance，极有可能所提交的partition的offset还是上一次(最新Rebalance之前的)此消费成员被分配的partition的offset
         * 比如：
         *  Rebalance 第1次 generationId: 1.0
         *    memberId-A 消费 topic-0、topic-1、topic-2 分区
         *    memberId-B 消费 topic-3、topic-4、topic-5 分区
         *
         * Rebalance 第2次 generationId: 2.0
         *   memberId-A 消费 topic-0、topic-1 分区
         *   memberId-B 消费 topic-3、topic-4 分区
         *   memberId-C 消费 topic-2、topic-5 分区
         *
         * 当第二次Rebalance后generationId是2.0，如果memberId-A消费成员带着generationId是1.0来提交offset，那就可能提交的是topic-0、topic-1、topic-2这些分区
         * 而此时topic-2分区在Rebalance第2次时，已经分配给新的成员memberId-C来消费了，所以如果memberId-A成员还提交topic-2分区的offset就不对了
         */
      } else if (generationId != group.generationId) {
        responseCallback(offsetMetadata.mapValues(_ => Errors.ILLEGAL_GENERATION)) // 返回此消费组成员的generation id无效的Response
      } else {
        val member = group.get(memberId) // 消费成员
        completeAndScheduleNextHeartbeatExpiration(group, member) // 该消费组的成员已经消费成功提交了offset，将心跳时间处理一下(归位)
        groupManager.storeOffsets(group, memberId, offsetMetadata, responseCallback) // 通过groupManager来存储offset等元数据信息
      }
    }
  }
  ...
  (Errors, Map[TopicPartition, OffsetFetchResponse.PartitionData]) = {
    ...
    validateGroupStatus(groupId, ApiKeys.OFFSET_FETCH) match {
      ...
      case None =>
        ...
        (Errors.NONE, groupManager.getOffsets(groupId, partitions)) // 获取相应的 offset 信息
    }
  }
