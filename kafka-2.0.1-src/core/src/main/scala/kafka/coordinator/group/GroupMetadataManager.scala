...
class GroupMetadataManager(brokerId: Int,
   ...
   *
   * 这个API提供的最重要的保证是，它永远不会返回过时的偏移量。即。，它要么返回当前偏移量，要么开始从日志同步缓存(并返回错误代码)。
   */
  def getOffsets(groupId: String, topicPartitionsOpt: Option[Seq[TopicPartition]]): Map[TopicPartition, OffsetFetchResponse.PartitionData] = {
    ...
    /**
     * 如果group是null，offset返回-1(INVALID_OFFSET)
     */
    if (group == null) {
      ...
      group.inLock { // group加锁
        if (group.is(Dead)) { // group的状态是Dead，offset返回-1(INVALID_OFFSET)
          topicPartitionsOpt.getOrElse(Seq.empty[TopicPartition]).map { topicPartition =>
            (topicPartition, new OffsetFetchResponse.PartitionData(OffsetFetchResponse.INVALID_OFFSET, "", Errors.NONE))
          }.toMap
        } else {
          topicPartitionsOpt match {
            case None => // 获取当前消费组所有topicPartition的offset
              ...
              //  返回此使用者组拥有的所有分区的偏移量。(这只适用于向Kafka提交偏移量的消费者。)
            ...
            case Some(topicPartitions) => // 获取当前消费组部分topicPartition的offset
              topicPartitions.map { topicPartition =>
                val partitionData = group.offset(topicPartition) match {
                  case None => // 如果没有offset
