...
/**
 * group 的 meta 信息,对 group 级别而言,每个 group 都会有一个实例对象
 */
private[group] class GroupMetadata(val groupId: String, initialState: GroupState) extends Logging {
  ...
  var generationId = 0
  private var leaderId: Option[String] = None // 组内 leader consumer id
  private var protocol: Option[String] = None

  private val members = new mutable.HashMap[String, MemberMetadata] // group 的 member 信息
  private val offsets = new mutable.HashMap[TopicPartition, CommitRecordMetadataAndOffset] // 对应的 commit offset
