...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handle(request: RequestChannel.Request) {
    try {
      ...
      request.header.apiKey match {
        ...
        case ApiKeys.OFFSET_COMMIT => handleOffsetCommitRequest(request) // 提供offset
        case ApiKeys.OFFSET_FETCH => handleOffsetFetchRequest(request) // 查询offset
  ...
  def handleOffsetFetchRequest(request: RequestChannel.Request) {
    val header = request.header // RequestHeader
    val offsetFetchRequest = request.body[OffsetFetchRequest]

    def authorizeTopicDescribe(partition: TopicPartition) =
      authorize(request.session, Describe, Resource(Topic, partition.topic, LITERAL)) // 验证tipic的权限

    def createResponse(requestThrottleMs: Int): AbstractResponse = {
      val offsetFetchResponse =
        // reject the request if not authorized to the group
        //  如果未授权给该消费组，则拒绝该请求
        if (!authorize(request.session, Describe, Resource(Group, offsetFetchRequest.groupId, LITERAL))) // 验证该消费组的权限
          offsetFetchRequest.getErrorResponse(requestThrottleMs, Errors.GROUP_AUTHORIZATION_FAILED) // 返回该消费组权限未验证通过的请求
        else { // 该消费组的权限验证通过
          if (header.apiVersion == 0) {
            ...
            //  版本0从ZK读取偏移量
            //  老版本是从ZK中读取Kafka的offset，下面的逻辑我们就不看了
            ...
            //  版本1及以上从Kafka读取偏移量
            //  新版本的Kafka是从 __consumer_offsets topic 来读取offset
            if (offsetFetchRequest.isAllPartitions) { // 获取这个消费组所有的topic-partition的offset
              val (error, allPartitionData) = groupCoordinator.handleFetchOffsets(offsetFetchRequest.groupId) // 实际 OFFSET_FETCH 请求处理
              if (error != Errors.NONE) // 如果处理异常
                offsetFetchRequest.getErrorResponse(requestThrottleMs, error) // 返回处理请求异常的Response
              else {
                // clients are not allowed to see offsets for topics that are not authorized for Describe
                //  如未授Describe权限，是不能查看offset的
                val authorizedPartitionData = allPartitionData.filter { case (topicPartition, _) => authorizeTopicDescribe(topicPartition) } // 过滤出有Describe权限的topicPartition
                new OffsetFetchResponse(requestThrottleMs, Errors.NONE, authorizedPartitionData.asJava) // 返回成功处理OFFSET_FETCH请求的Response
              }
            } else { // 获取这个消费组部分的topic-partition的offset
              val (authorizedPartitions, unauthorizedPartitions) = offsetFetchRequest.partitions.asScala
                .partition(authorizeTopicDescribe) // 返回经过授权与未经过授权的Partition
              val (error, authorizedPartitionData) = groupCoordinator.handleFetchOffsets(offsetFetchRequest.groupId,
                Some(authorizedPartitions)) // 实际 OFFSET_FETCH 请求处理，authorizedPartitions：已经授权的Partition
              if (error != Errors.NONE) // 如果处理异常
                offsetFetchRequest.getErrorResponse(requestThrottleMs, error) // 返回处理请求异常的Response
              else {
                val unauthorizedPartitionData = unauthorizedPartitions.map(_ -> OffsetFetchResponse.UNAUTHORIZED_PARTITION).toMap // 未经过授权的Partition包装Response响应
                new OffsetFetchResponse(requestThrottleMs, Errors.NONE, (authorizedPartitionData ++ unauthorizedPartitionData).asJava) // 返回成功处理OFFSET_FETCH请求的Response，与未经过授权Partition的Response响应
              }
            }
          }
        }
