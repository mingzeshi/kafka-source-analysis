...
class KafkaServer(val config: KafkaConfig, time: Time = Time.SYSTEM, threadNamePrefix: Option[String] = None,
   ...
   * 启动Kafka服务的单个实例
   *
   * 实例化LogManager、SocketServer和请求处理：KafkaRequestHandlers
   *
   *
   */
  def startup() {
    try {
      ...
      if (canStartup) {
        ...
        // Hardcode Time.SYSTEM for n｜ow as some Streams tests fail otherwise, it would be good to fix the underlying issue
        // 启动Coordinator
        groupCoordinator = GroupCoordinator(config, zkClient, replicaManager, Time.SYSTEM)
        groupCoordinator.startup() // 启动 groupCoordinator
