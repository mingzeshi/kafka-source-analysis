...
public final class ProducerBatch {
    ...
    public FutureRecordMetadata tryAppend(long timestamp, byte[] key, byte[] value, Header[] headers, Callback callback, long now) {
        if (!recordsBuilder.hasRoomFor(timestamp, key, value, headers)) { // 检查是否有足够的数据写入空间
            return null;
        } else {
            Long checksum = this.recordsBuilder.append(timestamp, key, value, headers); // 写入数据到MemoryRecords
            this.maxRecordSize = Math.max(this.maxRecordSize, AbstractRecords.estimateSizeInBytesUpperBound(magic(),
                    recordsBuilder.compressionType(), key, value, headers)); // 消息的最大值限制与key、value总大小，取最大值
            this.lastAppendTime = now; // 最后写入数据的时间
            FutureRecordMetadata future = new FutureRecordMetadata(this.produceFuture, this.recordCount,
                                                                   timestamp, checksum,
                                                                   key == null ? -1 : key.length,
                                                                   value == null ? -1 : value.length); // 发送数据的返回结果(未来的)
            // we have to keep every future returned to the users in case the batch needs to be
            // split to several new batches and resent.
            thunks.add(new Thunk(callback, future)); // 回调函数
            this.recordCount++; // 加入此RecordBatch的数量
            return future;
        }
    }
