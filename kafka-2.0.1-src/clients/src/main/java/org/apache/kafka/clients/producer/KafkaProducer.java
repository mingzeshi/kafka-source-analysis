...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    /**
     * KafkaProducer的初始化方法，它会创建以下的组件与信息：
     *
    ...
    KafkaProducer(ProducerConfig config,
        ...
        try {
            Map<String, Object> userProvidedConfigs = config.originals(); // 拿到一些配置信息
            this.producerConfig = config;
            this.time = Time.SYSTEM;
            String clientId = config.getString(ProducerConfig.CLIENT_ID_CONFIG); // 获取clientId
            if (clientId.length() <= 0) // 如果没有设置clientId，那就根据全局的变量来创建一个
                clientId = "producer-" + PRODUCER_CLIENT_ID_SEQUENCE.getAndIncrement();
            this.clientId = clientId;

            String transactionalId = userProvidedConfigs.containsKey(ProducerConfig.TRANSACTIONAL_ID_CONFIG) ?
                    (String) userProvidedConfigs.get(ProducerConfig.TRANSACTIONAL_ID_CONFIG) : null; // 事务ID，每台机器唯一
            ...
            Map<String, String> metricTags = Collections.singletonMap("client-id", clientId); // 这个应该是度量方式比如：求和、平均值
            MetricConfig metricConfig = new MetricConfig().samples(config.getInt(ProducerConfig.METRICS_NUM_SAMPLES_CONFIG))
                    .timeWindow(config.getLong(ProducerConfig.METRICS_SAMPLE_WINDOW_MS_CONFIG), TimeUnit.MILLISECONDS)
                    .recordLevel(Sensor.RecordingLevel.forName(config.getString(ProducerConfig.METRICS_RECORDING_LEVEL_CONFIG)))
                    .tags(metricTags); // 统计运行时期指标的配置信息
            List<MetricsReporter> reporters = config.getConfiguredInstances(ProducerConfig.METRIC_REPORTER_CLASSES_CONFIG, // metric.reporters
                    MetricsReporter.class); // 统计运行时期指标度量值的输出组件
            reporters.add(new JmxReporter(JMX_PREFIX)); // JmxReporter 是kafka内置的，统计并输出：运行时期、运行状况的指标组件
            this.metrics = new Metrics(metricConfig, reporters, time); // 创建组件，来支持producer的运行时期指标统计
            ProducerMetrics metricsRegistry = new ProducerMetrics(this.metrics);
            this.partitioner = config.getConfiguredInstance(ProducerConfig.PARTITIONER_CLASS_CONFIG, Partitioner.class); // 创建Partitioner，是生产者在生产数据的时候来做分区用的，默认是hashpartitioner
            long retryBackoffMs = config.getLong(ProducerConfig.RETRY_BACKOFF_MS_CONFIG); // 生产者重复发送消息，重试间隔默认是100ms
            if (keySerializer == null) { // key的序列化
            ...
            if (valueSerializer == null) { // value的序列化
            ...
            userProvidedConfigs.put(ProducerConfig.CLIENT_ID_CONFIG, clientId); // 设置 client.id
            List<ProducerInterceptor<K, V>> interceptorList = (List) (new ProducerConfig(userProvidedConfigs, false)).getConfiguredInstances(ProducerConfig.INTERCEPTOR_CLASSES_CONFIG,
                    ProducerInterceptor.class); // Kafka producer拦截器(interceptor)
             ...
             */
            this.maxRequestSize = config.getInt(ProducerConfig.MAX_REQUEST_SIZE_CONFIG); // max.request.size: 一个请求request中最大字节数，用于限制producer发送的单个请求request中，record batches的最大数量，以避免单个请求数据过于巨大
            this.totalMemorySize = config.getLong(ProducerConfig.BUFFER_MEMORY_CONFIG); // buffer.memory: 该参数用于指定Producer端用于缓存消息的缓冲区大小，单位为字节，默认值为：33554432合计为32M。kafka采用的是异步发送的消息架构，prducer启动时会首先创建一块内存缓冲区用于保存待发送的消息，然后由一个专属线程负责从缓冲区读取消息进行真正的发送。
            this.compressionType = CompressionType.forName(config.getString(ProducerConfig.COMPRESSION_TYPE_CONFIG)); // compression.type: producer压缩器，目前支持none（不压缩），gzip，snappy和lz4。

            this.maxBlockTimeMs = config.getLong(ProducerConfig.MAX_BLOCK_MS_CONFIG); // max.block.ms: 该参数指定了在调用 send() 方法或使用 partitionsFor() 方法获取元数据时生产者的阻塞时间。当生产者的发送缓冲区已满，或者没有可用的元数据时，这些方法就会阻塞。在阻塞时间达到 max.block.ms 时，生产者会抛出超时异常。
            this.requestTimeoutMs = config.getInt(ProducerConfig.REQUEST_TIMEOUT_MS_CONFIG); // request.timeout.ms 指定了生产者在发送数据时等待服务器返回响应的时间。
            this.transactionManager = configureTransactionState(config, logContext, log); // TODO shimingze
            int retries = configureRetries(config, transactionManager != null, log);
            int maxInflightRequests = configureInflightRequests(config, transactionManager != null);
            short acks = configureAcks(config, transactionManager != null, log); // 获取ack

            this.apiVersions = new ApiVersions(); // TODO shimingze

            /**
             * RecordAccumulator，缓冲区，负责消息的复杂的缓冲机制，发送到每个分区的消息会被打包成batch，一个broker上的多个分区对应的多个batch会被打包成一个request，batch size（16kb）
             *   默认情况下，如果光光是考虑batch的机制的话，那么必须要等到足够多的消息打包成一个batch，才能通过request发送到broker上去；但是有一个问题，如果你发送了一条消息，但是等了很久都没有达到一个batch大小
             *   所以说要设置一个linger.ms，如果在指定时间范围内，都没凑出来一个batch把这条消息发送出去，那么到了这个linger.ms指定的时间，比如说5ms，如果5ms还没凑出来一个batch，那么就必须立即把这个消息发送出去
             */
            this.accumulator = new RecordAccumulator(logContext,
                    config.getInt(ProducerConfig.BATCH_SIZE_CONFIG), // batch.size: 一个batch的大小，一般来说是要自己手动设置的，不是纯粹依靠默认值的，16kb
                    this.totalMemorySize, // 缓存消息的缓冲区大小
                    this.compressionType, // 数据压缩器
                    config.getLong(ProducerConfig.LINGER_MS_CONFIG), // linger.ms，如果在指定时间范围内，都没凑出来一个batch把这条消息发送出去，那么到了这个linger.ms指定的时间，那么就必须立即把这个消息发送出去
                    retryBackoffMs, // 生产者重复发送消息，每次重试间隔默认是100ms
                    metrics, // JmxReporter
                    time, // 系统时间
                    apiVersions,
                    transactionManager);
            List<InetSocketAddress> addresses = ClientUtils.parseAndValidateAddresses(config.getList(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG)); // bootstrap.servers broker集群的地址，producer通过这些与kafka集群建立链接
            if (metadata != null) {
                this.metadata = metadata; // TODO shimingze
            } else {
                this.metadata = new Metadata(retryBackoffMs, config.getLong(ProducerConfig.METADATA_MAX_AGE_CONFIG), // 元数据最大生存时间，每隔metadata.max.age.ms时间，producer客户端会强制刷新一遍元数据metadata，即使没有任何partition leadership主动发现新的broker或者新的partition。
                    true, true, clusterResourceListeners);
                this.metadata.update(Cluster.bootstrap(addresses), Collections.<String>emptySet(), time.milliseconds()); // 更新元数据信息
            }
            ChannelBuilder channelBuilder = ClientUtils.createChannelBuilder(config);
            Sensor throttleTimeSensor = Sender.throttleTimeSensor(metricsRegistry.senderMetrics);

            /**
             * 网络通信的组件，NetworkClient，一个网络连接最多空闲多长时间（9分钟），每个连接最多有几个request没收到响应（5个），
             * 重试连接的时间间隔（50ms），Socket发送缓冲区大小（128kb），Socket接收缓冲区大小（32kb）
             */
            KafkaClient client = kafkaClient != null ? kafkaClient : new NetworkClient( // 创建网络通信的客户端
                    new Selector(config.getLong(ProducerConfig.CONNECTIONS_MAX_IDLE_MS_CONFIG), // connections.max.idle.ms: 用于设置连接最大的空闲时间，超过这个时间没有活动的broker连接，则关闭
                            this.metrics, time, "producer", channelBuilder, logContext),
                    this.metadata, // 元数据，用来找topic所在的broker
                    clientId, // 客户端ID
                    maxInflightRequests,
                    config.getLong(ProducerConfig.RECONNECT_BACKOFF_MS_CONFIG), // reconnect.backoff.ms: 重连间隔时间，避免producer客户端过于紧密循环地重连kafka服务broker。该值针对的是所有client到broker的连接。
                    config.getLong(ProducerConfig.RECONNECT_BACKOFF_MAX_MS_CONFIG), // reconnect.backoff.max.ms
                    config.getInt(ProducerConfig.SEND_BUFFER_CONFIG),  // send.buffer.bytes: TCP发送缓冲区（SO_SNDBUF）的大小，若send.buffer.bytes设为-1，则使用操作系统的默认值。
                    config.getInt(ProducerConfig.RECEIVE_BUFFER_CONFIG), // receive.buffer.bytes: TCP接收缓冲区（SO_RCVBUF）大小，当receive.buffer.bytes设置为-1,则使用操作系统默认的大小。
                    this.requestTimeoutMs, // 阻塞等待超时时间
                    time,
                    true,
                    apiVersions,
                    throttleTimeSensor,
            ...
            /**
             * Sender线程，负责从缓冲区里获取消息发送到broker上去，request最大大小（1mb），acks（1，只要leader写入成功就认为成功），重试次数（0，无重试），请求超时的时间（30s），
             * 线程类叫做“KafkaThread”，线程名字叫做“kafka-producer-network-thread”，此处线程直接被启动
             */
            this.sender = new Sender(logContext,
                    client,
                    this.metadata,  // 元数据信息，用来封装每个batch应该往哪个broker上的哪个topic-partition发数据
                    this.accumulator, // producer端的数据缓冲区，producer是先把数据写入这个缓冲区的，然后再有sender线程来处理数据，打成一个一个的batch包，封装以每个broker为单位的request包，一个broker也就是一台机器发一次
                    maxInflightRequests == 1, // max.in.flight.requests.per.connection
                    config.getInt(ProducerConfig.MAX_REQUEST_SIZE_CONFIG), // max.request.size: 该参数用于控制生产者发送的请求大小。它可以指能发送的单个消息的最大值，也可以指单个请求里所有消息总的大小。例如，假设这个值为 1MB，那么可以发送的单个最大消息为 1MB，或者生产者可以在单个请求里发送一个批次，该批次包含了 1000 个消息，每个消息大小为 1KB。另外，broker 对可接收的消息最大值也有自己的限制（message.max.bytes），所以两边的配置最好可以匹配，避免生产者发送的消息被 broker 拒绝。
                    acks, // acks
                    retries, // retries
                    metricsRegistry.senderMetrics,
                    Time.SYSTEM, // 时间
                    this.requestTimeoutMs, // 阻塞等待超时时间
                    config.getLong(ProducerConfig.RETRY_BACKOFF_MS_CONFIG), // 每次尝试增加的额外的间隔时间
                    this.transactionManager,
                    apiVersions);
            String ioThreadName = NETWORK_THREAD_PREFIX + " | " + clientId;
            this.ioThread = new KafkaThread(ioThreadName, this.sender, true); // 创建sender线程的信息
            this.ioThread.start(); // 启动sender线程持续监听RecordAccumulator如果有数据，如果到达batch大小，或超过一定时间，就打包发送到broker上去
    ...
    private static int configureRetries(ProducerConfig config, boolean idempotenceEnabled, Logger log) {
        ...
        if (config.originals().containsKey(ProducerConfig.RETRIES_CONFIG)) { // retries
        ...
        if (idempotenceEnabled && config.getInt(ProducerConfig.RETRIES_CONFIG) == 0) { // retries
            throw new ConfigException("Must set " + ProducerConfig.RETRIES_CONFIG + " to non-zero when using the idempotent producer.");
        }
        return config.getInt(ProducerConfig.RETRIES_CONFIG); // retries
    }
    ...
    private static int configureInflightRequests(ProducerConfig config, boolean idempotenceEnabled) {
        ...
        if (idempotenceEnabled && 5 < config.getInt(ProducerConfig.MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION)) { // max.in.flight.requests.per.connection
            throw new ConfigException("Must set " + ProducerConfig.MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION + " to at most 5" +
                    " to use the idempotent producer.");
        }
        return config.getInt(ProducerConfig.MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION); // max.in.flight.requests.per.connection
    }
    ...
    private static short configureAcks(ProducerConfig config, boolean idempotenceEnabled, Logger log) {
        ...
        if (config.originals().containsKey(ProducerConfig.ACKS_CONFIG)) { // acks
