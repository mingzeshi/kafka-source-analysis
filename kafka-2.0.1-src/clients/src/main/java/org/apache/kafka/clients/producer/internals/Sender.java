...
public class Sender implements Runnable {
     ...
     *
     * Sender线程的核心方法，负责从缓冲区accumulator里获取消息发送到broker上去，request最大大小（1mb），acks（1，只要leader写入成功就认为成功），
     * 重试次数（0，无重试），请求超时的时间（30s），线程类叫做“KafkaThread”，线程名字叫做“kafka-producer-network-thread”
     *
    ...
    void run(long now) {
        ...
        client.poll(pollTimeout, now); // TODO shimingze
    }

    private long sendProducerData(long now) {
        Cluster cluster = metadata.fetch(); // TODO shimingze
